	...
	<view class="post-village">
		...
		<form @submit="submit">
			...
			<view class="cu-form-group">
				...
				<input type="text" v-model="labelName" placeholder="请输入标签名称" />
				<button class="cu-btn sm bg-sea" @tap="addTag">添加</button>
			</view>
			<view class="cu-form-group flex-wrap justify-start tags" v-if="tags.length">
				<view class="cu-capsule radius" v-for="(item, idx) in tags" :key="idx">
					<view class="cu-tag bg-red" @tap="deleteTag(idx)"><view class="cuIcon-roundclose"></view></view>
					<view class="cu-tag line-red">{{ item }}</view>
				</view>
			</view>
			<view class="cu-form-group">
	...
	export default {
		...
		methods: {
			...
			submit (e) {
				...
				if (checkRes) {
					this.params.label = this.tags.join(',')
